...
export default function CollegeCard({ college, initialSaved = false }: CollegeCardProps) {
  ...
  const toggleSave = async (e: React.MouseEvent) => {
    e.preventDefault(); // Stop click from triggering parent Link navigation
    
    if (!session) {
      // Direct unauthorized guests to register/login instantly
      router.push('/login');
      return;
    }
  ...
  const formattedFees = (college.fees / 100000).toFixed(1); // Converts to Lakhs per year format
  ...
  return (
    <div className="group bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 flex flex-col overflow-hidden relative">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        {/* Detailed Page Routing Action Trigger */}
        <Link
